    ...
    fn bishop_moves(self, position: u8, _board: Board) -> Vec<u8> {
     ...
     fn knight_moves(self, position:u8, board: Board) -> Vec<u8> {
        let possible_positions: Vec<u8> = vec![
            position.checked_add(COL).and_then(|x| x.checked_add(2 * ROW)),
            position.checked_sub(COL).and_then(|x| x.checked_add(2 * ROW)),
            position.checked_add(COL).and_then(|x| x.checked_sub(2 * ROW)),
            position.checked_sub(COL).and_then(|x| x.checked_sub(2 * ROW)),
            position.checked_add(ROW).and_then(|x| x.checked_add(2 * COL)),
            position.checked_add(ROW).and_then(|x| x.checked_sub(2 * COL)),
            position.checked_sub(ROW).and_then(|x| x.checked_add(2 * COL)),
            position.checked_sub(ROW).and_then(|x| x.checked_sub(2 * COL)),
        ]
        .into_iter()
        .flatten()
        .collect();
